...
class AssetManager:
    ...
    def _download_and_extract(
        ...
        try:
            ...
            with open(temp_archive_path, "wb") as out_file:
                ...
                if temp_archive_path.suffix == ".zip":
                    with zipfile.ZipFile(temp_archive_path, "r") as zf:
                        ...
                        for i, file in enumerate(zf.namelist()):
                            zf.extract(file, path=destination_dir)
                            progress.update(extract_task, advance=100 / file_count)
                    ...
                    with tarfile.open(temp_archive_path, "r:gz") as tf:
                        ...
                        for i, member in enumerate(members):
                            ...
                            progress.update(extract_task, advance=100 / len(members))
    ...
    def ensure_uv_available(self) -> bool:
        ...
        if not asset_info:
            logging.error(
                f"Your OS/architecture ({system}/{machine}) is not automatically supported."
            )
            self.console.print(
                f"[red]✗ Your OS/architecture ({system}/{machine}) is not supported for automatic 'uv' installation.[/red]"
            )
            return False
        ...
        return self._download_and_extract(
            download_url, asset_name, Config.UV.DIR, post_extract_uv
        )
    ...
    def _setup_logging(self):
        ...
        class RichLogHandler(logging.Handler):
            ...
            def emit(self, record):
                ...
                level_color = color_map.get(record.levelname, "white")
                formatted = f"[{level_color}]{record.levelname}[/]: {log_entry}"
...
"""
    ...
    def _manage_core(self) -> bool:
        ...
        return self.asset_manager.install_core()
    ...
    def _prompt_for_app_installation(self) -> bool:
        ...
        for line in content.splitlines():
            if line.strip().upper().startswith("APP_"):
                try:
                    ...
                    if len(parts) >= 3:
                        name, folder, repo = parts[0], parts[1], parts[2]
        ...
        if table is not None:
            table.add_column("Num", style="cyan", justify="center")
            table.add_column("Application Name", style="green")
            table.add_column("Repository", style="dim")
            for i, app in enumerate(apps):
                repo_short = "/".join(app["repo_url"].split("/")[-2:])
                table.add_row(str(i + 1), app["name"], repo_short)
    ...
    def _launch_application(self) -> None:
        ...
        self.console.print(
            f"\n[bold blue]🚀 Launching {runner_path.parent.name} application...[/bold blue]\n"
        )
